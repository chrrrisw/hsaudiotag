...
def squeeze_mpeg(inpath, outpath):
    ...
    if not frames:
        print u'Couldn\'t find frames', fb.position
    frame_count = len(frames) // 4
    result += (u'[SQUEEZED_FRAMES:%d]' % frame_count).encode(u'ascii')
    result += frames
    result += s[last_pos:] # add whatever's after the mpeg frames
    outfile = open(outpath, u'wb')
    outfile.write(result)

def expand_mpeg(filename):
    # takes the squeezed `filename`, expands it, and returns a file-like object
    infile = open(filename, u'rb')
    s = infile.read()
    re_squeezed = re.compile(ur'\[SQUEEZED_FRAMES:(\d+?)\]')
    match = re_squeezed.search(unicode(s, u'latin-1'))
    if match is None:
        return BytesIO(s)
    result = BytesIO()
    ...
    for i in xrange(frame_count):
        ...
        assert h.valid
        frames.append(frame_data_string + ('\0' * (h.size - 4)))
    frames = ''.join(frames)
    result.write(frames)
...
def squeeze_mp4(inpath, outpath):
    ...
    s = open(inpath).read()
    m = mp4.File(inpath)
    [mdat] = [a for a in m.atoms if (a.size > 8) and (a.type == u'mdat')]
    ...
    result = u''.join([before, u'[SQUEEZED_BYTES:%d]' % data_size, after])
    outfile = open(outpath, u'w')
    outfile.write(result)
...
def expand_mp4(filename):
    ...
    infile = open(filename, u'rb')
    s = infile.read()
    re_squeezed = re.compile(ur'\[SQUEEZED_BYTES:(\d+?)\]')
    match = re_squeezed.search(unicode(s, u'latin-1'))
    if match is None:
        return BytesIO(s)
    result = BytesIO()
