...
class StubReader(io.BytesIO):
    u"""This class is to allow myself to remove the .seek .read code from mp4.Atom
...
class TestMp4Atom(object):
    u"""Test mp4.Atom class.
    ...
    def test_minimal(self):
        u"""A 8 bytes 'aaaa' atom"""
        atom = self.CreateAtom('\x00\x00\x00\x08aaaa')
        eq_(atom.valid,True)
        eq_(atom.size,8)
        eq_(atom.type,u'aaaa')

    def test_wrong_size(self):
        u"""A 8 bytes 'aaaa' atom with a reported size of 12
    ...
    def test_alphabet(self):
        u"""An 'alph' atom with the whole alphabet in it."""
        atom = self.CreateAtom('\x00\x00\x00\x22alphabcdefghijklmnopqrstuvwxyz')
        eq_(atom.valid,True)
        eq_(atom.size,34)
        eq_(atom.type,u'alph')
        eq_(atom.read(0,10),'abcdefghij')
        eq_(atom.read(0),'abcdefghijklmnopqrstuvwxyz')

    def test_withinbox(self):
        u"""Test the attributes of an Atom that is within another atom"""
    ...
    def test_data3(self):
        ...
        eq_(atom.data[2],'aybabtu')

    def test_data4(self):
        u"""Test the data of an atom. a str field at the end, but with a len descriptor"""
        atom = self.CreateAtom('\x00\x00\x00\x13data\x01\x02\x03\x04aybabtu')
    ...
    def test_data5(self):
        u"""Test the data of an atom. a str field at the end, but with a len descriptor that is not long enough"""
        atom = self.CreateAtom('\x00\x00\x00\x13data\x01\x02\x03\x04aybabtu')
        atom.cls_data_model = '2H4s'
        eq_(atom.data[0],0x0102)
        eq_(atom.data[1],0x0304)
        eq_(atom.data[2],'ayba')
        eq_(len(atom.data),3)

    def test_data6(self):
        u"""Test the data of an atom. a str field at the end, but with a len descriptor that is too long"""
        atom = self.CreateAtom('\x00\x00\x00\x13data\x01\x02\x03\x04aybabtu')
...
class TestMp4AtomBox(object):
    u"""Test mp4.AtomBox class.
    ...
    def test_box1(self):
        u"""A 'box1' atom with 4 subatoms in it."""
    ...
    def test_box2(self):
        u"""A 'box2' atom with 3 subatoms in it and some data before."""
        atom = self.CreateAtom('\x00\x00\x00\x42box1\x00\x00\x00\x08sub1\x00\x00\x00\x08sub2\x00\x00\x00\x08sub3\x00\x00\x00\x22sub4abcdefghijklmnopqrstuvwxyz')
        atom.cls_data_model = u'2H4s'
        eq_(len(atom.atoms),3)
        eq_(atom.data[0],0x0000)
        eq_(atom.data[1],0x0008)
        eq_(atom.data[2],'sub1')
        eq_(atom.atoms[0].type,u'sub2')

class TestMp4FileTest1(object):
    def setup_method(self, method):
        self.file = mp4.File(expand_mp4(TestData.filepath(u'mp4/test1.m4a')))
    ...
    def test_atom_4(self):
        ...
        eq_(u'mdat'  ,atom.type)
        eq_(atom.start_offset + atom.size, atom.parent.size)

    def test_find(self):
        eq_(self.file.atoms[1],self.file.find(u'moov'));
        eq_(None,self.file.find(u'bleh'));
        eq_(self.file.atoms[1].atoms[2].atoms[0].atoms[1].atoms[4],self.file.find(u'moov.udta.meta.ilst.gnre'));

    def test_valid(self):
        eq_(self.file.valid,True)

    def test_info(self):
        eq_(u'This Is How It Goes',self.file.title)
        eq_(u'Billy Talent',self.file.artist)
        eq_(u'Billy Talent',self.file.album)
        eq_(u'Punk',self.file.genre)
        eq_(u'',self.file.comment)
        eq_(3364781,self.file.size)
        eq_(44100,self.file.sample_rate)
        eq_(207,self.file.duration)
        eq_(128,self.file.bitrate)
        eq_(u'2003',self.file.year)
    ...
    def test_cant_open(self):
        ...
        del self.file
        open(TestData.filepath(u'mp4/test1.m4a'),u'r+b') # no crash
        mp4.File(TestData.filepath(u'mp4/test1.m4a')) # no crash
...
class TestMp4Filezerofile(object):
    def setup_method(self, method):
        self.file = mp4.File(TestData.filepath(u'zerofile'))

    def test_find(self):
        eq_(None,self.file.find(u'moov'));
        eq_(None,self.file.find(u'bleh'));
        eq_(None,self.file.find(u'moov.udta.meta.ilst.gnre'));

    def test_valid(self):
        eq_(self.file.valid,False)

    def test_info(self):
        eq_(u'',self.file.title);
        eq_(u'',self.file.artist);
        eq_(u'',self.file.album);
        eq_(u'',self.file.genre);
        eq_(u'',self.file.comment);
...
class TestMp4Filerandomfile(TestMp4Filezerofile):
    def setup_method(self, method):
        self.file = mp4.File(TestData.filepath(u'randomfile'))

class TestMp4Filezerofill(TestMp4Filezerofile):
    def setup_method(self, method):
        self.file = mp4.File(TestData.filepath(u'zerofill'))

class TestMp4Fileinvalid1(object):
    def setup_method(self, method):
        self.file = mp4.File(TestData.filepath(u'mp4/invalid1.m4a'))

    def test_find(self):
        eq_(self.file.atoms[0],self.file.find(u'mvhd'));
...
def test_file_test2():
    f = mp4.File(TestData.filepath(u'mp4/test2.m4a'))
    eq_(u'Intro to Where It\'s At',f.title)
    eq_(u'Beck',f.artist)
    eq_(u'Odelay',f.album)
    eq_(u'Alternative',f.genre)
    eq_(u'This is a test',f.comment)
    eq_(754518,f.size)
...
def test_file_test3():
    f = mp4.File(expand_mp4(TestData.filepath(u'mp4/test3.m4a')))
    assert isinstance(f.title,unicode)
    assert isinstance(f.artist,unicode)
    assert isinstance(f.title,unicode)
    eq_(u'J\'ai oubli\u00e9',f.title)
    assert isinstance(f.artist,unicode)
    eq_(u'Capitaine R\u00e9volte',f.artist)
    eq_(u'Danse sociale',f.album)
    eq_(u'Punk',f.genre)
    eq_(u'',f.comment)
    eq_(3813888,f.size)
    eq_(44100,f.sample_rate)
...
def test_file_test4():
    f = mp4.File(expand_mp4(TestData.filepath(u'mp4/test4.m4a')))
    eq_(u'2005', f.year)

def test_file_test5():
    f = mp4.File(expand_mp4(TestData.filepath(u'mp4/test5.m4a')))
    eq_(u'Hip Hop/Rap', f.genre)
    eq_(128, f.bitrate)
...
def test_file_test6():
    ...
    f = mp4.File(expand_mp4(TestData.filepath(u'mp4/test6.m4p')))
    eq_(128, f.bitrate)
...
def test_file_test7():
    ...
    f = mp4.File(expand_mp4(TestData.filepath(u'mp4/test7.m4a')))
    eq_(u'Coolio', f.artist)
    eq_(u'That\'s How It Is', f.title)
    eq_(u'Gangsta\'s Paradise', f.album)
    eq_(u'Hip Hop/Rap', f.genre)
    eq_(0, f.bitrate)
    eq_(60, f.duration)

def test_non_ascii_genre():
    fp = mp4.File(TestData.filepath(u'mp4/non_ascii_genre.m4a'))
    eq_(fp.genre, u'\xe9')

def test_genre_index_out_of_range():
    # Don't crash when a file has a numerical genre that is out of range
    fp = mp4.File(TestData.filepath(u'mp4/genre_index_out_of_range.m4a'))
    eq_(fp.genre, u'') # don't crash
...
def test_empty_attribute_atom():
    ...
    fp = mp4.File(TestData.filepath(u'mp4/empty_attribute_atom.m4a'))
    eq_(fp.title, u'') # don't crash
...
def test_invalid_utf8():
    ...
    fp = mp4.File(TestData.filepath(u'mp4/invalid_utf8.m4a'))
    eq_(fp.artist, u'Billy Talen')
