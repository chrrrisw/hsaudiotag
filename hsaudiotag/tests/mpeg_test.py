...
class TestMpeg(object):
    def test1(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test1.mp3')))
        assert m.valid
        eq_(m.size, 2355703)
        eq_(m.bitrate, 128)
        eq_(m.duration, 147)
    ...
    def test2(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test2.mp3')))
        eq_(m.size,2353527)
        eq_(m.bitrate,128)
        eq_(m.duration,147)
    ...
    def test3(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test3.mp3')))
        eq_(m.size,2701840)
        eq_(m.bitrate,128)
        eq_(m.duration,168)
        assert m.id3v1.exists
    ...
    def test4(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test4.mp3')))
        eq_(m.size,2702480)
        eq_(m.bitrate,128)
        eq_(m.duration,168)
        assert m.id3v1.exists
    ...
    def test5(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test5.mp3')))
        eq_(m.size,2701840)
        eq_(m.bitrate,128)
        eq_(m.duration,168)
        assert m.id3v1.exists
    ...
    def test6(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test6.mp3')))
        assert m.vbr
        eq_(m.size,11198817)
        eq_(m.bitrate,202)
        eq_(m.duration,443)
    ...
    def testwith_footer(self):
        ...
        # tag is 0x20a
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'id3v2/with_footer.mp3')))
        eq_(m.size,2702234)
        eq_(m.bitrate,128)
        eq_(m.duration,168)
        assert not m.id3v1.exists
    ...
    def testZeroFile(self):
        m = mpeg.Mpeg(TestData.filepath(u'zerofile'))
        assert not m.valid
        eq_(m.size,0)
        eq_(m.bitrate,0)
        eq_(m.duration,0)
    ...
    def test7(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test7.mp3')))
        assert m.vbr
        eq_(m.bitrate,172)
        eq_(m.duration,182)
        assert m.id3v1.exists
    ...
    def test8(self):
        ...
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/test8.mp3')))
        eq_(m.duration,299)
    
    def test_double_id3(self):
        #This file has 2 ID3v2 tags with one garbage char in the middle of them. CBR
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/double_id3.mp3')))
        eq_(m.duration,203)
        eq_(128,m.bitrate)
    
    def test_vbr_without_header(self):
        #Same as test6, but with the Xing header frame removed.
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/vbr_without_header.mp3')))
        assert m.vbr
        eq_(m.bitrate,202)
        eq_(m.duration,443)
        assert m.id3v1.exists
    ...
    def test_vbr_fhg(self):
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/vbr_fhg.mp3')))
        assert m.vbr
        eq_(m.bitrate,221)
        eq_(m.duration,194)
    
    def test_vbr_xing(self):
        m = mpeg.Mpeg(expand_mpeg(TestData.filepath(u'mpeg/vbr_xing.mp3')))
        assert m.vbr
        eq_(m.bitrate,211)
        eq_(m.duration,193)
    ...
    def test_tag_duration_different(self):
        ...
        m = mpeg.Mpeg(TestData.filepath(u'mpeg/tag_duration_different.mp3'))
        eq_(m.duration, 1)
    

class TestFrameBrowser(object):
    def test_valid_first_frame_and_no_tag(self):
        fp = expand_mpeg(TestData.filepath(u'mpeg/test3.mp3'))
        b = mpeg.FrameBrowser(fp)
        eq_(0,b.frame_index)
        eq_(128,b.frame.bitrate)
        eq_(128,b.next().bitrate)
        eq_(1,b.frame_index)
        fp.close()
    
    def test_must_seek_first_frame(self):
        fp = expand_mpeg(TestData.filepath(u'mpeg/test2.mp3'))
        b = mpeg.FrameBrowser(fp)
        eq_(128,b.frame.bitrate)
        eq_(128,b.next().bitrate)
        fp.close()
    ...
    def test_First(self):
        fp = expand_mpeg(TestData.filepath(u'mpeg/test6.mp3'))
        b = mpeg.FrameBrowser(fp)
    ...
    def test_seek_with_id3_tag_in_the_way(self):
        ...
        fp = expand_mpeg(TestData.filepath(u'mpeg/double_id3.mp3'))
        b = mpeg.FrameBrowser(fp)
        assert b.frame.valid
        fp.close()
    
    def test_zerofile(self):
        fp = open(TestData.filepath(u'zerofile'),u'rb')
        b = mpeg.FrameBrowser(fp)
        eq_(0,b.frame_index)
    ...
    def test_stats_on_one_second(self):
        ...
        fp = open(TestData.filepath(u'mpeg/one_second.mp3'), u'rb')
        b = mpeg.FrameBrowser(fp)
        fcount, size = b.stats()
        eq_(fcount, 39)
        eq_(size, 39 * 417)
